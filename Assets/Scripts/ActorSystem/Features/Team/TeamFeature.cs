...
public sealed class TeamFeature : MonoBehaviour, IActorFeature
{
    ...
    public void Initialize(ActorContext ctx)
    {
        if (ctx == null)
        {
            Debug.LogError("TeamFeature.Initialize: ctx is null", this);
            enabled = false;
            return;
        }
        if (ctx.Definition == null)
        {
            Debug.LogError("TeamFeature.Initialize: ctx.Definition is null", this);
            enabled = false;
            return;
        }
    ...
    public bool IsFriendly(TeamFeature otherTeam)
    {
