...
public sealed class ActorKernel : MonoBehaviour {
    ...
    [SerializeField] private ActorEventBus eventBus = new();
    ...
    void Awake() {
        ...
        foreach (var f in _features) f.Initialize(Ctx);
    }
    void Update() { float dt = Time.deltaTime; for (int i=0;i<_features.Length;i++) _features[i].Tick(dt); }
    void FixedUpdate(){ float fdt = Time.fixedDeltaTime; for (int i=0;i<_features.Length;i++) _features[i].FixedTick(fdt); }
    void OnDestroy(){ for (int i=0;i<_features.Length;i++) _features[i].Shutdown(); }
    public void SetDefinition(ActorDefinition def) { // runtime swap support (variants/upgrades)
        definition = def;
    }
}